        ...
        <aside class="sidebar fixed top-0 left-0 h-full z-30 flex flex-col">
            ...
            <nav class="flex-1 px-3 py-4 overflow-y-auto">
                ...
                <a href="{{ route('member.dashboard') }}"
                    class="nav-item {{ request()->routeIs('member.dashboard') && !request('tab') ? 'active' : '' }}">
                    <i data-lucide="layout-dashboard"></i> Dashboard
                </a>

                <div class="nav-section">Peminjaman</div>
                <a href="{{ route('member.dashboard') }}?tab=pinjam"
                    class="nav-item {{ request('tab') === 'pinjam' ? 'active' : '' }}">
                    <i data-lucide="book-plus"></i> Pinjam Buku
                </a>
                <a href="{{ route('member.dashboard') }}?tab=aktif"
                    class="nav-item {{ request('tab') === 'aktif' ? 'active' : '' }}">
                    <i data-lucide="arrow-left-right"></i> Buku Dipinjam
                </a>
                <a href="{{ route('member.dashboard') }}?tab=riwayat"
                    class="nav-item {{ request('tab') === 'riwayat' ? 'active' : '' }}">
                    <i data-lucide="history"></i> Riwayat
                </a>

                <div class="nav-section">Keuangan</div>
                <a href="{{ route('member.dashboard') }}?tab=denda"
                    class="nav-item {{ request('tab') === 'denda' ? 'active' : '' }}">
            ...
            <div class="p-3 border-t border-slate-800">
                <div class="flex items-center gap-3 px-2 py-2">
                    <div class="w-8 h-8 rounded-full bg-sky-600 flex items-center justify-center text-white text-xs font-bold flex-shrink-0">
                        {{ strtoupper(substr(optional(Auth::user())->nama_lengkap ?? optional(Auth::user())->username ?? 'DU', 0, 2)) }}
                    </div>
                    <div class="flex-1 min-w-0">
                        <p class="text-slate-200 text-xs font-medium truncate">{{ optional(Auth::user())->nama_lengkap ?? optional(Auth::user())->username ?? 'Demo User' }}</p>
                        <p class="text-slate-500 text-xs truncate">{{ optional(optional(Auth::user())->anggota)->nomor_anggota ?? 'Anggota' }}</p>
                    </div>
